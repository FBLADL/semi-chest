...
class Moco_lincls_single(object):
    def __init__(self) -> None:
        ...
        self.net1, self.net2 = self.create_model(
            backbone, args.num_class, args.mlp)
    ...
    def deploy(self):
        ...
        self.warmup_criterion1 = ELR_Plus(
            len(labeled_loader.dataset), beta=args.beta)
        self.warmup_criterion2 = ELR_Plus(
            len(labeled_loader.dataset), beta=args.beta)
        ...
        if args.pretrained:
            ...
            for k in list(state_dict.keys()):
                ...
                elif k.startswith('module.encoder_q.mlp_kept') or k.startswith('module.encoder_q.mlp2_kept'):
                    state_dict[k[len('module_encoder_q.'):]] = state_dict[k]
                del state_dict[k]
            args.start_epoch = 0
        ...
        for warmup in range(args.start_epoch, args.epochs):
            ...
            state_dict1, early_targets1 = self.train(self.warmup_criterion1, warmup, self.net1, self.net1_ema,
                                                     self.optimizer1,
                                                     labeled_loader, unlabeled_loader,
                                                     net_ema2=self.net2_ema, optimizer_ema=self.optimizer1_ema)
            # self.lr_scheulder1.step(
            state_dict2, early_targets2 = self.train(self.warmup_criterion2, warmup, self.net2, self.net2_ema,
                                                     self.optimizer2,
                                                     labeled_loader, unlabeled_loader,
                                                     net_ema2=self.net1_ema, optimizer_ema=self.optimizer2_ema)
            state_dict = {'epoch': warmup,
                          'net1': state_dict1, 'net2': state_dict2, 'et1': early_targets1, 'et2': early_targets2}
    ...
    def pseudo_label(self, epoch):
        self.net1_ema.eval()
        self.net2_ema.eval()

    def train(self, criterion, epoch, net, net_ema, optimizer, loader_x, loader_u, net_ema2=None, optimizer_ema=None):
        net.train()
        ...
        for batch_idx, (inputs_x, labels, item) in loader:
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                criterion.update_hist(outputs_x_ema, item.numpy(
                ).tolist(), mix_index=mix_index, lamb=lamb)
                loss, elr_reg = criterion(
                    item, outputs_x.float(), labels.float())
                total_loss = loss + args.elr_weight * elr_reg + weight * mse
    ...
    def create_optimizer(self):
        return torch.optim.Adam(list(filter(lambda p: p.requires_grad, self.net1.parameters())),
                                lr=args.lr,
                                betas=(0.9, 0.99),
                                eps=0.1), torch.optim.Adam(
            list(filter(lambda p: p.requires_grad, self.net2.parameters())), lr=args.lr, betas=(0.9, 0.99), eps=0.1)
