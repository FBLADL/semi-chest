...
def download_chexpert_unzip(data_dir: str):
    bucket_prefix = 'chexpert.zip'
    dst_folder = Path(data_dir)
    print('| dst folder... {}'.format(dst_folder))
    ...
    if dst_folder.exists():
        ...
        bucket = get_bucket('aiml-carneiro-research',
                            'aiml-carneiro-research-data')
        blob = bucket.blob(f'fb/{bucket_prefix}')
        with open(Path('/pvc')/bucket_prefix, 'wb') as sink:
            blob.download_to_file(sink)
        print('unziping')
        print(os.listdir('/pvc/'))
        try:
            with zipfile.ZipFile('/pvc/chexpert.zip', 'r') as zip_ref:
                zip_ref.extractall('/pvc/')
        except FileExistsError:
            print('file alreayd exists {}'.format('pvc'))
    print(os.listdir('/pvc/'))

def download_isic_unzip(data_dir:str):
    bucket_prefix = 'ISIC2018.zip'
    dst_folder = Path(data_dir)
    print('| dst folder... {}'.format(dst_folder))
    ...
    if dst_folder.exists():
        ...
        print(os.listdir('/pvc/'))
        try:
            with zipfile.ZipFile('/pvc/ISIC2018.zip', 'r') as zip_ref:
                zip_ref.extractall('/pvc/')
        except FileExistsError:
            print('file alreayd exists {}'.format('pvc'))
    print(os.listdir('/pvc/'))
